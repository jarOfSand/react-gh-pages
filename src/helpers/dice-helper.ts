...
export function rollHandfull(handfullString: string, critMode: boolean = false): number[] {
    ...
    const handfullResults = handfullArray.map((substring) => {
        if (substring.includes('d')) {
            ...
            const diceQuant = (diceQuantString === '' ? 1 : parseInt(diceQuantString)) * (critMode ? 2 : 1);
